...
export function ProductGrid({
  ...
  if (products.length === 0) {
    return (
      ...
      >
        <div className="mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-[var(--surface-subtle)] text-[var(--text-muted)]">
          <svg
            className="h-6 w-6"
            fill="none"
            stroke="currentColor"
        ...
        {hasActiveFilters && onReset && (
          <button
            ...
            onClick={onReset}
            className="mt-5 inline-flex items-center gap-1.5 rounded-[var(--radius-sm)] bg-[var(--accent)] px-4 py-2 text-xs font-semibold text-white shadow-sm transition-colors hover:bg-[var(--accent-hover)] focus:ring-2 focus:ring-[var(--accent)] focus:ring-offset-2 focus:outline-none"
